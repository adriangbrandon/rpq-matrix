...
#define CHECKRESULTS 0   //if set to 1, it checks the results of the queries against the expected ones.
#define VERBOSETESTS 0   //if set to 1, it prints the results of each query launched.



namespace rangeRowGenerator {
    /* see also prepare-range-row-queries.cpp */
    ...
    public:
       ...
       static bool readNextPattern(std::ifstream &input, uint32_t &row, uint32_t & nresults) {
            std::string line;
            if (std::getline(input, line)) {
                std::stringstream ss(line);
                char sep1; //#
                //line format:   rowVal #nresults
                ss >> row >> sep1 >> nresults;
                return true;
            }
            return false;
        }

        /* REads nqueries non-empty row queries from .query (row val) and .query.res (resulting points for query)*/
        static uint64_t read_row_queries(const std::string &file_queries,
            ...
            uint32_t row, nresults;
            uint64_t tres=0;

           uint64_t max_nresults = 0;
            while ( readNextPattern(input_query, row,  nresults) ) {
                std::vector<uint32_t> pat;
                pat.push_back(row);pat.push_back(nresults);
                tres += nresults;
                if (nresults>max_nresults) max_nresults = nresults;
                Q.push_back(pat);   //row, nresults
            }
            input_query.close();
            queries = Q;

            uint32_t nqueries = Q.size();
            std::cout << "\t *" << nqueries <<"* Row queries Read from '" << file_queries+ ".query" << "':: Avg-points=" << tres/nqueries << "."<< std::endl;
            return max_nresults;
        }
       ...
       /* Creates nqueries non-empty row queries and saves them into .query (row) and .query.res (points in query results)*/
       static void read_row_queries_PointSets(const std::string &file_queries,  std::vector<std::vector<std::pair<uint32_t, uint32_t>>> &pointSets){
           ...
           if (nqueries)
                std::cout << "\t *" << nqueries <<"* Row queries Read from '" << file_queries+ ".query.res" << "':: Avg-points=" << tres/nqueries << "."<< std::endl;
        }

       static void print_row_query_points(std::vector<std::vector<std::pair<uint32_t, uint32_t>>> &pointSets, uint32_t i) {
...
int main(int argc, char **argv) {
    ...
    for(auto & file : files) {
        ...
        std::vector<std::vector<uint32_t>> queries;
        uint64_t tmpmax= rangeRowGenerator::reader::read_row_queries(queries_dir +  "/" + util::file::remove_extension(file), queries);
        if (tmpmax>max_nresults) max_nresults = tmpmax;
        queriesForMatrices.push_back(queries);


        #if CHECKRESULTS ==1
        //Read the results expected for the row-queries for this matrix
        std::vector<std::vector<std::pair<uint32_t, uint32_t>>> pointSets;
        rangeRowGenerator::reader::read_row_queries_PointSets(queries_dir + "/" + util::file::remove_extension(file), pointSets);
    ...
    std::cout << " Max_results-row-query-buffer =  <" << max_nresults << " results>" << std::endl;
    ...
    std::cout << "Row Queries..." << std::flush;
    sum = 0;
    ...
    for(uint z = 0; z < nmatrices; z++) {
        ...
        uint64_t i;
        //run all the row-queries over the k2-tree
        uint32_t nqueries = queries.size();
        uint64_t nresults = 0;
        for (i=0; i<nqueries; i++) {
            uint64_t nresultsExp = queries[i][1];
            count_results =0;
            ...
            count_results = wrapper::collect(k2tree,queries[i][0], queries[i][0], 0, k2tree->width,results_buffer);
            //count_results /= 2;
            ...
            { //check results for k2_tree.range_query_buf
                ...
                for (uint64_t j=0; j<count_results; j++) {
                    if (pairs[j].first != pointSets[i][j].first || pairs[j].second != pointSets[i][j].second) {
                        std::cout << "\t \t Error: Query-line: row =" << queries[i][0] << " #nresults:" <<queries[i][1] << std::endl;
    ...
    std::cerr << "Row queries: total = " << totNqueries << " queries, total results =" << sum<< std::endl;
    ...
    #if CHECKRESULTS ==1
    std::cout << "\t TEST: done. [ row-queries ] "<<totNqueries << ", (width = " << queriesForMatrices[0][0][1] - queriesForMatrices[0][0][0]+1 << ")  TESTs SUCCEEDED - !" << std::endl;
    #endif
