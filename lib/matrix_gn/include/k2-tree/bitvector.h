...
extern inline uint bitsAccess (bitvector B, uint64_t i);
	...
	// a more basic version on arrays 
inline uint bitsAccessA (uint64_t *B, uint64_t i);

        // read bits [i..i+l-1], l <= w
inline uint64_t bitsReadA (uint64_t *B, uint64_t i, uint l);

        // counts # of 1s in y
inline uint popcount (uint64_t y);
	// popcount in a nibble
inline uint pop4 (uint x);
...
inline uint64_t bitsRank (bitvector B, uint64_t i);
